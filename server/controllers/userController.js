...
const {User, Basket} = require('../dbmodels/models')
...
class UserController {
    async registation(req,res,next) {
        ...
        if(!email && !password) {
            return next(ApiError.badRequest('Не корректные вводимые данные'))
            }
            ...
            if(condidate) {
             ...
             const user = await User.create({email, role, password: HashedPass})
